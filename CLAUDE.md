...
- **`data/preprocess.py`** — Data preprocessing stub (raw → processed). Raw data must be added manually the first time; after that, use `dvc pull` to restore on other machines.
...
- **`params.yaml`** — All hyperparameters: image_size (default 224x224 RGB), num_classes, batch_size, lr, max_epochs, num_workers, release S3 settings. Reduce batch_size or image_size if hitting GPU OOM.
...
- **Model Export**: ONNX + onnxruntime (optional group: `poetry install --with export`)
- **CV Extras**: timm, albumentations, pycocotools (optional group: `poetry install --with cv`)
...
- The Dockerfile uses a multi-stage build with CUDA 12.8 base, installs deps via pip from an exported `requirements.txt`, and runs training directly (`python -m {module_name}.train`). Use `docker_build.sh` to export requirements and build the image.
