...
if (process.env.SKIP_VENV_INSTALL === '1') {
  // Override poetry installDependencies method immediately after project creation
  if (project.poetry) {
    const originalInstallDependencies = project.poetry.installDependencies;
    if (typeof originalInstallDependencies === 'function') {
      project.poetry.installDependencies = function() {
        console.log('⚠️  Skipping automatic dependency installation during bootstrap.');
        console.log('   Install manually later with: poetry install');
  ...
  if (installTask) {
    ...
    project.addTask('install', {
      exec: 'echo "⚠️  Skipping automatic dependency installation. Install manually with: poetry install"',
...
// Note: pyproject.toml is automatically generated by projen when poetry: true is set
// Poetry handles package metadata and dependencies in pyproject.toml
// No need for setup.py or requirements.txt
...
safeSampleFile(`src/${moduleName}/config.py`, 'config.py');
safeSampleFile(`src/${moduleName}/utils.py`, 'utils.py');
